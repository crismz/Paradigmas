...
public class QuickHeuristic extends Heuristic{
	...
	private static List<String> keyWords = List.of(
		    ...
		    "hi", "hello",
			// Commom found words
			"criticized", "requires", "manufacturing", "wireless" , "allowing", "detailed",
			"shooting", "infrastructure", "start", "something", "new", "low", "shares",
			"zero","one","two","three","four","five","six","seven","eight","nine","rip",
			"continued", "dating", "commission", "deal", "trying", "running", "debt", "lucrative"
			);
	...
	public static void main(List<String> candidates) {
		
		QuickHeuristic qh = new QuickHeuristic();

		List<NamedEntity> namedEntities = qh.getNamedEntitiesList(qh, candidates);

		for (NamedEntity name: namedEntities) {
			name.prettyPrint();
		}
	}
}
